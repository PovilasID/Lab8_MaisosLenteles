 ...
 * @author Juliaus
 */
public class MapT<Key, Value> implements MapADTp<Key, Value> {
    ...
    public MapT(HashType ht) {
        this(DEFAULT_INITIAL_CAPACITY, ht);
    }

    public MapT(int initialCapacity, HashType ht) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR, ht);
    }

    public MapT(float loadFactor, HashType ht) {
        this(DEFAULT_INITIAL_CAPACITY, loadFactor, ht);
    }

    public MapT(int initialCapacity, float loadFactor, HashType ht) {
...
//==============================================================================
    private void rehash(Node<Key, Value> node) {
        MapT mapKTU =
                new MapT(table.length * 2, loadFactor, ht);
        for (int i = 0; i < table.length; i++) {
            while (table[i] != null) {
