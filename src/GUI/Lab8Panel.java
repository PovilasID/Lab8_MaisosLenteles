...
import lab8_povilas_sidaravicius.GreitaveikosTyrimas;
import lab8_povilas_sidaravicius.Place;
import lab8_povilas_sidaravicius.PlaceGenerator;
import studijosKTU.*;
...
//  |-----------------------------------------------------------------------|   
    ...
    public void totalGaps() {
        KsSwing.oun(taEvents, "Tuščių elementų skaičius: " + map.emptyElementsNumber());
    }

    public void chains() {
        KsSwing.oun(taEvents, "Vidutinis ilgis: " + map.chainLengthAverage());
    }

    public void searchAvg() {
        String laikinasId = panParam3.getParametersOfTable()[0].replace("-", "x");
        Place bi = new Place();
        bi = map.get(laikinasId);
        KsSwing.oun(taEvents, "Praeita elementu: " + map.containsValueX(bi));
    }
    ...
    public void mapGeneration(String fName) throws MyException {
        ...
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<String, Place>(new String(), new Place(), initialCapacity, loadFactor, ht);
                break;
            case 1:
                map = new MapTiesinisx<String, Place>(new String(), new Place(), initialCapacity, loadFactor, ht2);
                break;
            //...
        ...
        if (fName == null) {
            autoArray = gamyba.makeAndSell(sizeOfSet, sizeOfGenSet);
            String abc;
            for (Place a : autoArray) {
                abc = gamyba.formuotiAutoID();
                id[numeris++] = abc;
                map.put("" + abc, a);
                KsSwing.oun(taEvents, a, rb.getStringArray("msgs")[1]);
            }
            enableButtons(true);
            //Jei automobiliai skaitomi iš failo
        } else {
            map.load(fName);
            enableButtons(false);
            KsSwing.oun(taEvents, rb.getStringArray("msgs")[6]);
        }
        table.setMyTable(map, map.getMaxChainSize(), map.getTableCapacity(), colWidth, delimiter);
        updateParameters(false);
    }

    public void mapAdd() throws MyException {
        Place a = gamyba.parduotiIsSandelio();
        map.put(
                "" + gamyba.formuotiAutoID() //raktas
                , a);
        table.setMyTable(map, map.getMaxChainSize(), map.getTableCapacity(), colWidth, delimiter);
    ...
    public void chainLengthAvg() {
        KsSwing.oun(taEvents, "Grandinėlių ilgių vidurkis: " + map.chainLengthAverage());
    }
        ...
        try {
            ...
            switch (cmbHashFunctions.getSelectedIndex()) {
                ...
                case 1:
                    ...
                    ht2 = MapTiesinis.HashType.Multiplication;
                    break;
                case 2:
                    ht = MapKTU.HashType.JavaCollectionsF;
                    ht2 = MapTiesinis.HashType.JavaCollectionsF;
                    break;
                default:
                    ht = MapKTU.HashType.Division;
                    break;
    ...
    private MapADTx<String, Place> map;
    private int sizeOfSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private String delimiter = "";
    private MapTiesinis.HashType ht2 = MapTiesinis.HashType.Division;
    private PlaceGenerator gamyba = new PlaceGenerator();
    private String[] id = new String[100];
    private int numeris = 0;
}